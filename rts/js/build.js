...
function spawnUnitNear(type, team, b){
  ...
  for(let dx=-1;dx<=b.w;dx++) for(let dy=-1;dy<=b.h;dy++){
    ...
    if(tx>=0&&ty>=0&&tx<MAP_W&&ty<MAP_H && !structBlocked[tx][ty] && tx>=b.tx-1 && tx<b.tx+b.w+1 && ty>=b.ty-1 && ty<b.ty+b.h+1){
      if(d.naval){
        if(terrain[tx][ty]==='water') cells.push([tx,ty]);          // 驱逐舰出生在水面
      } else if(d.amphib){
        if(terrain[tx][ty]==='water'||terrain[tx][ty]==='grass') cells.push([tx,ty]);  // 运输艇可上岸
      } else if(!blocked[tx][ty]) cells.push([tx,ty]);
    }
  }
  if(cells.length){
