...
function buildingStatsHTML(b){
  ...
  if(b.defName==='factory' && b.upgraded) h+=statRow('升级','已升级');
  if(b.upgrading) h+=statRow('升级', Math.floor(b.upgradeProg/FACTORY_UPGRADE_TIME*100)+'%');
    ...
    if(selBuilding.team===TEAM_A){
      if(selBuilding.constructing){ mkAction('建造中...','none',false); }
        ...
        if(selBuilding.defName==='factory'){
          if(selBuilding.upgraded){ mkUnit(advancedTankType(TEAM_A)); mkUnit('mcv'); }
